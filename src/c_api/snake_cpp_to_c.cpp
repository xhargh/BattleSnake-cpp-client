...
void snake_c_callbacks_set(const SnakeCallbacks * const pSnake,
    void *pUserData) {
  sctc_ps = pSnake;
  sctc_pu = pUserData;
}
...
nlohmann::json battlesnake_start(const std::string& game_id, const Index width,
    const Index height) {

  std::cout << "*** New game started *** width=" << width << ", height="
      << height << ", id=" << game_id << ".\n";
  ...
  StartOutputT startOutput = { "red", "white", "Defaulty McDefaultFace",
      "I'm going default on your **bleep**", SH_SMILE, ST_FRECKLED };
...
Move_response battlesnake_move(const std::string& game_id, const Index width,
    const Index height, const Points& food, const Snakes& snakes,
    const Snakes& dead_snakes, const size_t my_snake_index) {
  ...
  for (size_t s = 0; s < snakes.size(); s++) {
    ...
    strncpy(sout.name, snake.name.c_str(),
        MIN(snake.name.length(), SNAKE_STRLEN));
    strncpy(sout.taunt, snake.taunt.c_str(),
        MIN(snake.taunt.length(), SNAKE_STRLEN));
